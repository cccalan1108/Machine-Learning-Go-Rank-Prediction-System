...
def parse_train_file_to_moves(fpath: Path, d_label: int):
    """return list of (gid:str, label:int, df_moves) per game"""
...
def _base_frame_to_step_matrix(df_moves: pd.DataFrame) -> np.ndarray:
    ...
    arg_p  = (np.argmax(A_p, axis=1).astype(np.float32) / 8.0)[:,None]
    arg_v  = (np.argmax(A_v, axis=1).astype(np.float32) / 8.0)[:,None]
    arg_r  = (np.argmax(A_r, axis=1).astype(np.float32) / 8.0)[:,None]
    ...
    blunder = (dlead < -5.0).astype(np.float32)                                
    ...
    for i, a in enumerate(parts):
        if a.ndim == 1:
            a = a[:, None]
        if a.ndim != 2:
            raise ValueError(f"[Step-Feature Shape Error] part index={i}, shape={a.shape} (expect 2D).")
        normed_parts.append(a.astype(np.float32, copy=False))

    step = np.concatenate(normed_parts, axis=1)
...
def compute_meta_side_features(df: pd.DataFrame) -> np.ndarray:
    ...
    def _ent(base):
        ...
        if not set(cols).issubset(df.columns) or n == 0: return 0.0
    ...
    def _seg_entropy(base, a, b):
        cols = [f'{base}_{i}' for i in range(1,10)]
        if n==0 or not set(cols).issubset(df.columns): return 0.0
        arr = df.iloc[a:b][cols].to_numpy(dtype=np.float32, copy=False)
        if arr.size == 0: return 0.0
        return float(_entropy_rows(arr).mean())

    h0,h1 = _seg_idx(0.00,0.33); m0,m1 = _seg_idx(0.33,0.66); t0,t1 = _seg_idx(0.66,1.00)
    seg_feats = [
...
class SeqDS(Dataset):
    def __init__(self, items, le:LabelEncoder, max_len:int, mean=None, std=None, crop='rand'):
        self.items = items; self.le = le
        self.max_len = max_len; self.crop=crop
    ...
    def _center_crop_pad(self, x):
        ...
        if t > self.max_len:
            ...
            x = x[s:s+self.max_len]; t = self.max_len
        out = np.zeros((self.max_len, x.shape[1]), dtype=np.float32)
        take = min(t, self.max_len); out[:take] = x[:take]
        return out
    ...
    def __getitem__(self, idx):
        ...
        if self.mean is not None and self.std is not None:
            mu = np.asarray(self.mean).reshape(1, -1)
            sd = np.asarray(self.std).reshape(1, -1)
            x = (x - mu) / (sd + 1e-6)
...
def compute_norm(items, max_len):
    ...
    for _,_,df in items:
        ...
        n  += x.shape[0]
    mean = s / max(1,n)
    var  = ss / max(1,n) - mean*mean
    std  = np.sqrt(np.maximum(var, 1e-8))
    return mean.astype('float32'), std.astype('float32'), int(feat_dim)
...
class BiLSTMClf(nn.Module):
    def __init__(self, in_dim, hidden=192, layers=2, dropout=0.2, num_class=9):
        super().__init__()
...
class TinyTransformer(nn.Module):
    def __init__(self, in_dim, d_model=192, nhead=6, layers=2, dim_ff=512, dropout=0.15, num_class=9):
        super().__init__()
...
def seq_fit_fold(model, train_items, val_items, le, max_len, mean, std, device,
                 epochs=30, lr=3e-4, weight_decay=1e-2, patience=6, batch_size=128):
    ...
    ld_tr = DataLoader(ds_tr, batch_size=batch_size, shuffle=False, sampler=sampler, num_workers=0)
    ...
    for ep in range(1, epochs+1):
        ...
        for _, X, y, m in ld_tr:
            ...
            y_bin = torch.zeros(y.size(0), K-1, device=y.device)
            for k in range(K-1): y_bin[:,k] = (y > k).float()
            loss_bce = criterion_bce(out['ord_logits'], y_bin)
            loss = 0.7*loss_bce + 0.3*loss_ce
            loss.backward()
        ...
        # EMA 驗證
        with torch.no_grad():
            shadow = ema.state_dict(); cur = model.state_dict()
            model.load_state_dict(shadow, strict=True); model.eval()
            ys, ps = [], []
            for _, X, y, m in ld_va:
                X = X.to(device); y = y.to(device); m = m.to(device)
                out = model(X, m)
                p = 0.7*coral_logits_to_proba(out['ord_logits']) + 0.3*torch.softmax(out['ce_logits'], dim=1)
                ps.append(p.argmax(dim=1).cpu().numpy()); ys.append(y.cpu().numpy())
        ...
        if acc > best_acc + 1e-6:
            ...
            es += 1
        print(f"[SEQ] val_acc={acc:.4f}  best={best_acc:.4f}  (epoch {ep})")
...
def _safe_skew(a):
    ...
    from scipy.stats import skew; return float(skew(a))
...
def _topk_entropy_features(df, base, prefix):
    ...
    if not set(cols).issubset(df.columns) or df.empty:
        return {f'{prefix}_{base}_entropy_mean':0.0,
    ...
    try:
        ...
        for f,(tr,va) in enumerate(skf.split(X,y),1):
            clf = CatBoostClassifier(
                depth=8, learning_rate=0.05, l2_leaf_reg=6.0, iterations=1500,
        ...
        for f,(tr,va) in enumerate(skf.split(X,y),1):
            clf = HistGradientBoostingClassifier(max_depth=None, learning_rate=0.05,
                                                l2_regularization=1e-3, max_leaf_nodes=127,
                                                random_state=100+f)
...
def main():
    ...
    print("計算標準化（動態 feat_dim）")
    mean, std, feat_dim = compute_norm(items, args.seq_len)
    print(f"feat_dim = {feat_dim}")

    print("K-fold OOF（Transformer / BiLSTM）")
    ...
    for f,(tr,va) in enumerate(folds,1):
        ...
        # Transformer
        tf = TinyTransformer(in_dim=feat_dim, d_model=224, nhead=7, layers=2, dim_ff=640,
        ...
        acc_tf, state_tf = seq_fit_fold(tf, tr_items, va_items, le, args.seq_len, mean, std, device,
                                        epochs=args.epochs, lr=args.lr, batch_size=args.batch_size, patience=7)
        cv_tf.append(acc_tf)
        p_tf = seq_predict_proba(tf, state_tf, va_items, le, args.seq_len, mean, std, device)
        oof_tf[va] = p_tf
        # BiLSTM
        bl = BiLSTMClf(in_dim=feat_dim, hidden=224, layers=2, dropout=0.2, num_class=num_class).to(device)
    ...
    print(f"TF mean={np.mean(cv_tf):.4f}  BL mean={np.mean(cv_bl):.4f}")
    ...
    print("Meta 準備")
    side_feats = np.vstack([compute_meta_side_features(df) for _,_,df in items])  # [N, 7 + 擴充]
    meta_X = np.concatenate([oof_tf, oof_bl, oof_tab, side_feats], axis=1)
    meta_scaler = StandardScaler().fit(meta_X)
    meta_Xs = meta_scaler.transform(meta_X)
    meta = LogisticRegression(multi_class='multinomial', solver='lbfgs', C=2.0, max_iter=400)
    ...
    print("全資料重訓（雙 seed ensemble）")
    def train_full(seed):
        set_seed(seed)
        tf_full = TinyTransformer(in_dim=feat_dim, d_model=224, nhead=7, layers=2, dim_ff=640,
                                  dropout=0.15, num_class=num_class).to(device)
        _, tf_state = seq_fit_fold(tf_full, items, items, le, args.seq_len, mean, std, device,
                                   epochs=max(10, args.epochs//2), lr=args.lr, batch_size=args.batch_size, patience=5)
        bl_full = BiLSTMClf(in_dim=feat_dim, hidden=224, layers=2, dropout=0.2, num_class=num_class).to(device)
    ...
    payload = {
        ...
        'seq_hparams': {
            ...
            'tf':{'hidden':224,'heads':7,'layers':2,'ff':640,'dropout':0.15},
            'bl':{'hidden':224,'layers':2,'dropout':0.2}
        },
        'seq_states': {
            'tf': [tf_s1, tf_s2],
            'bl': [bl_s1, bl_s2]
